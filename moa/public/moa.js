    ...
    */
    angular.module('moaDirective', [])
        ...
        .controller('moaTreeController', ['$scope', '$http', '$compile', function($scope, $http, $compile){
            ...
            //节点模板
            var template =
                '<ul class="list-group" data-ng-show="!node.collapsed" style="margin-bottom:0px;">' +
                    '<li data-ng-repeat="child in node.children" class="list-group-item" style="padding-left: 10px;padding-right: 0px; padding-bottom : 0px; padding-top:0px; border: 0px;">'+
                        '<i data-ng-hide="child.leaf" data-ng-click="child.expandCollapse(child)" class="{{child.collapsed?expandicon:collapseicon}}"></i>' +
                        '<i data-ng-show="child.leaf" style="padding-left:20px;"></i>' +
            ...
            $scope.init = function(element){
                ...
                loadChildren(node, function(node, children){
                    var temp =
                        ...
                        '<div class="panel-heading" ng-show="title != undefined && title != \'\'">{{title}}</div>' +
                        '<div class="panel-body">' + template + '</div>'+
                        '</div>';
                    element.html('').append($compile(temp)($scope));
                });
            }
